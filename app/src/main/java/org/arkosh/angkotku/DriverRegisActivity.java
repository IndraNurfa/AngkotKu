...
public class DriverRegisActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        fAuthStateListener = firebaseAuth -> {
            ...
            if (user != null) {
                Intent intent = new Intent(DriverRegisActivity.this, DriverHomepageActivity.class);
        ...
        cancel.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent = new Intent(DriverRegisActivity.this, DriverLogRegActivity.class);
                startActivity(intent);
                finish();
                return;
            }
    ...
    private void result() {
        if (TextUtils.isEmpty(email.getText().toString()) || TextUtils.isEmpty(name.getText().toString())
            ...
            Toast.makeText(DriverRegisActivity.this, "Harus isi semua", Toast.LENGTH_LONG).show();
            return;
        }
        if (TextUtils.isEmpty(email.getText().toString())) {
            Toast.makeText(DriverRegisActivity.this, "Email tidak boleh kosong", Toast.LENGTH_LONG).show();
            return;
        }
        if (TextUtils.isEmpty(name.getText().toString())) {
            Toast.makeText(DriverRegisActivity.this, "Nama tidak boleh kosong", Toast.LENGTH_LONG).show();
            return;
        }
        if (pass.getText().toString().length() < 6 || pass.getText().toString().length() < 6) {
            Toast.makeText(DriverRegisActivity.this, "Harus 6 karakter atau lebih", Toast.LENGTH_LONG).show();
            return;
        }
        if (!pass.getText().toString().equals(passConf.getText().toString())) {
            Toast.makeText(DriverRegisActivity.this, "Password Harus Sama", Toast.LENGTH_LONG).show();
            return;
        }
        ...
        mAuth.createUserWithEmailAndPassword(mEmail, mPass).addOnCompleteListener(DriverRegisActivity.this, new OnCompleteListener<AuthResult>() {
            @Override
            public void onComplete(@NonNull @org.jetbrains.annotations.NotNull Task<AuthResult> task) {
                if (!task.isSuccessful()) {
                    Toast.makeText(DriverRegisActivity.this, "Registrasi Error!", Toast.LENGTH_SHORT).show();
                } else {
